...
def get_yes24_detail_page_url(book_name: str) -> Optional[str]:
    ...
    url = (
        f"https://www.yes24.com/searchcorner/Search?keywordAd=&keyword=&domain=BOOK&"
...
class Yes24ScrapResult:
    ...
    def get_author(self) -> Optional[str]:
        tag_author = (
            "#yDetailTopWrap > div.topColRgt > div.gd_infoTop > "
    ...
    def get_page_count(self) -> Optional[int]:
        tag_page = (
            "#infoset_specific > div.infoSetCont_wrap > div > table > "
        ...
        try:
            ...
            if "쪽" in page_count_plus_etc and "확인" not in page_count_plus_etc:
                ...
                return int(
                    "".join([char for char in page_count_str if char.isnumeric()])
                )
        except AttributeError:
            pass
    ...
    def get_cover_image_url(self) -> Optional[str]:
        tag_cover_image = (
            "#yDetailTopWrap > div.topColLft > div.gd_imgArea > " "span > em > img"
        )
        try:
...
def parse_contents(contents_html: str):
    ...
    for line in striped:
        line_frags = [
            line[i : i + MAX_LINE_LENGTH] for i in range(0, len(line), MAX_LINE_LENGTH)
        ]

...
from app.my_block import DatabaseEnum, progress, record_timestr_prop, \
    ...
    record_to_set_prop, record_to_reading_prop, reading_to_main_date_prop, \
    ...
    record_to_stage_prop, record_to_scrap_prop, \
    record_to_thread_prop, record_to_idea_prop, record_to_gist_prop, relevant, \
...
class MatchRecordDateiByTitle(MatchSequentialAction):
    ...
    def process_page(self, record: Page) -> Any:
        if record.parent != self.record_db:
            return
        if not (datei := self.date_namespace.get_page_by_record_title(
...
class MatchEventProgress(MatchSequentialAction):
    ...
    def _determine_forward_prog(self, event: Page) -> Optional[RelationPagePropertyValue]:
        stage_list = [target for target in event.properties[record_to_set_prop]
...
class CopyEventProgressRels(MatchSequentialAction):
    ...
    def process_page(self, event: Page) -> Any:
        ...
        for rel_prop in [record_to_progress_datei_prop, record_to_stage_prop, record_to_idea_prop,
                         record_to_thread_prop, record_to_set_prop, record_to_reading_prop,

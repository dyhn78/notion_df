...
from notion_df.property import PageProperties, RelationProperty, RelationPagePropertyValue, \
    DualRelationDatabasePropertyValue, RelationDatabasePropertyValue, DualRelationProperty
...
def subtract_db(target_db: DatabaseEnum):
    ...
    for target in target_db.entity.query(
        rel_prop1.filter.is_not_empty()
    ):
        ...
        if target_rel1.has_more:
            ...
            # ENDTODO
        target_rel1_minus_rel2 = target.properties[rel_prop1] - target.properties[rel_prop2]
...
def update_page(target: Page, prop: RelationProperty, new_prop_value: RelationPagePropertyValue):
    ...
    target.update(
        PageProperties(
            {
                prop: prop.page_value(new_prop_value[:max_relation_length])
            }
        )
    )
            ...
            PageProperties(
                {
                    synced_prop: synced_prop.page_value(
                        counterpage.properties[synced_prop]
                        + [target]
                    )
                }
            )

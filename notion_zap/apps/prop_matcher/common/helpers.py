...
def find_unique_unarchived_id_from_relation(
        dom: editors.PageRow, targets: editors.PageList, to_tar: str) -> str:
...
def find_all_unarchived_id_from_relation(
        dom: editors.PageRow, targets: editors.PageList, to_tar: str) -> list[str]:
    tar_ids = dom.props.read_at(to_tar)
    res = []
    for tar_id in tar_ids:
...
def find_unique_target_id_by_homo_ref(
        dom: editors.PageRow, domain: editors.PageList, target: editors.PageList, to_tar: str):

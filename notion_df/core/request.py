...
class RequestBuilder(metaclass=ABCMeta):
    ...
    all non-abstract subclasses must provide class type argument `ContentsT`.
...
class SingleRequestBuilder(Generic[ContentsT], RequestBuilder, metaclass=ABCMeta):
    data_type: type[ContentsT]
    ...
    @classmethod
    def parse_response_data(cls, data: dict[str, Any]) -> ContentsT:

...
class SearchByTitle(PaginatedRequestBuilder[Union[PageData, DatabaseData]]):
    ...
    sort: TimestampSort = field(
        default_factory=lambda: TimestampSort("last_edited_time", "descending")
    )
    page_size: int | None = None

    def get_settings(self) -> RequestSettings:
        return RequestSettings(Version.v20220628, Method.POST, "search")
        ...
        return DictFilter.not_none(
            {
                "query": self.query,
                "filter": (
                    {"value": self.entity, "property": "object"}
                    if self.entity
                    else None
                ),

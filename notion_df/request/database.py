...
class RetrieveDatabase(SingleRequestBuilder[DatabaseData]):
    ...
    def get_settings(self) -> RequestSettings:
        return RequestSettings(Version.v20220628, Method.GET, f"databases/{self.id}")
...
class CreateDatabase(SingleRequestBuilder[DatabaseData]):
    ...
    def get_settings(self) -> RequestSettings:
        return RequestSettings(Version.v20220628, Method.POST, "databases/")
...
class UpdateDatabase(SingleRequestBuilder[DatabaseData]):
    ...
    def get_settings(self) -> RequestSettings:
        return RequestSettings(Version.v20220628, Method.PATCH, f"databases/{self.id}")
...
class QueryDatabase(PaginatedRequestBuilder[PageData]):
    ...
    def get_settings(self) -> RequestSettings:
        return RequestSettings(
            Version.v20220628, Method.POST, f"databases/{self.id}/query"
        )

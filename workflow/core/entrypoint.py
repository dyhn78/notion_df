...
class WorkflowRecord:
    ...
    def __init__(self, *, update_last_success_time: bool):
        ...
        self.last_execution_time_str = (cast(ParagraphBlockValue, last_execution_time_block.latest.value)
    ...
    def __exit__(self, exc_type: type, exc_val, exc_tb) -> None:
        ...
        for block in reversed(self.page_block.retrieve_children()):
            if isinstance(block.latest.value, DividerBlockValue):
                log_group_block = self.page_block.append_children([
                    ToggleBlockValue(RichText([TextSpan(self.start_time_group_str)]))])[0]
                break
            if cast(ToggleBlockValue, block.latest.value).rich_text.plain_text == self.start_time_group_str:
                log_group_block = block
                break
            if self.start_time - block.latest.created_time > timedelta(days=7):
                block.delete()

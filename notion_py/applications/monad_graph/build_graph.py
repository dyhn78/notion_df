...
from notion_py.applications.monad_graph.self_related_dataframe \
    import SelfRelatedPageListDeprecated, SelfRelatedDatabaseFrame, theme_dataframe, idea_dataframe


class BuildGraph:
    def __init__(self, page_size: int):
        self.themes = theme_dataframe.query_all_deprecated(page_size=page_size)
        self.ideas = idea_dataframe.query_all_deprecated(page_size=page_size)
    ...
    def add_edges(self):
        for down_pagelist in self.all.values():
            assert isinstance(down_pagelist.frame, SelfRelatedDatabaseFrame)

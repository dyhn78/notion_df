...
class MatchBase:
    def __init__(self, caller: DateMatcher, database_info: tuple,
        ...
        self.gateway: TypeName.query = self.pagelist.query_gateway
    ...
    def query_as_parents(self):
        if self.date_range:
            frame = self.gateway.filter_maker.by_date(self.index_as_domain)
    ...
    def query_as_children(self, dom_to_tar):
        frame = self.gateway.filter_maker.by_relation(dom_to_tar)
        ft = frame.is_empty()
        if self.date_range:
            frame = self.gateway.filter_maker.by_date(self.index_as_domain)
            if self.date_range == 7:

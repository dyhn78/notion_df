...
from ..inline.core import ChildBearingBlock
...
class PageList(Editor):
    ...
    def run_query(self, page_size=0):
        response = self.query_gateway.execute(page_size=page_size)
...
class TabularPageBlock(ChildBearingBlock):
    ...
    def read(self):
        return {'type': 'page',
                'tabular': self.props.read_at_all(),
                'children': self.children.reads()}

    def read_rich(self):
        return {'type': 'page',
                'tabular': self.props.read_rich_at_all(),
